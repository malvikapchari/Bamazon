...
var start = function() {
  ...
  var query = Bamazon.makeQuery(col);
  connection.query(query, function(err, res) {
    makeQuery(res);
  });
};

var makeQuery = function(res) {
  ...
  inquirer.prompt([{
    ...
    validate: function(value) {
      if (value>=0 && value<=max && value%1 === 0 && value.indexOf(' ')<0 && value.indexOf('.')<0) {
        ...
        return 'Type a number between 1 and ' + max + ' without no spaces';
      }
    }
  } , {
    name: "quantity",
...
var checkQuantity = function(answer) {
  ...
  connection.query(query, params, function(err, res) {
    if (res[0].StockQuantity < answer.quantity) {
      console.log(chalk.bold.red('Not enough in stock! We have only ' + res[0].StockQuantity) + '.');
      chooseItem(max);
    } else {
...
var updateQuantity = function(id,total,newQuantity) {
  ...
  connection.query(query, params, function(err, res) {
    ...
    console.log(chalk.bold.blue('Thanks for shopping!'));
  });
};
